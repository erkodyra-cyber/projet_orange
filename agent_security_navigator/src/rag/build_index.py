...
PROCESSED = Path("agent_security_navigator/data/processed/chunks.jsonl")
...
def l2(x: np.ndarray)->np.ndarray:
    if x.ndim != 2 or x.size == 0:
        raise ValueError("Vecteurs vides : lance pdf_to_chunks.py et vérifie les PDFs.")
    n = np.linalg.norm(x, axis=1, keepdims=True); n[n==0]=1e-12; return x/n

def main():
    texts, metas = [], []
    if not PROCESSED.exists():
        raise FileNotFoundError(f"{PROCESSED} introuvable. Lance d’abord pdf_to_chunks.py")
    ...
    print(f"OK: {len(texts)} passages indexés → {INDEX_DIR}")
