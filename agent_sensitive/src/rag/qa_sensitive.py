...
# --- Config
INDEX_DIR = Path("agent_sensitive/index_sensitive")  # <- adapté au sensible
MISTRAL_API_KEY = os.getenv("MISTRAL_API_KEY")
CHAT_MODEL = os.getenv("CHAT_MODEL", "mistral-small-latest")
EMBED_MODEL = os.getenv("EMBED_MODEL", "intfloat/multilingual-e5-base")

# Limites & timeouts
CTX_CHAR_LIMIT = int(os.getenv("CTX_CHAR_LIMIT", "1200"))
MISTRAL_TIMEOUT_S = float(os.getenv("MISTRAL_TIMEOUT_S", "45"))
MISTRAL_RETRIES = int(os.getenv("MISTRAL_RETRIES", "1"))
...
def load_resources() -> Dict[str, Any]:
    ...
    if not store_p.exists() or not faiss_p.exists():
        raise FileNotFoundError("Index sensible absent. Lance pdf_to_chunks_sensitive.py puis build_index_sensitive.py")
...
def build_messages(question: str, passages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    sys = (
        "Tu es un assistant RAG sur des documents sensibles.\n"
